...
public class UserController extends BaseAction {
    ...
    @GetMapping("/role/{id}")
    public Message getUserRoleList(@PathVariable String id) {

        String roles = userService.loadAccountRole(id);
    ...
    public Message authorityUserRole(HttpServletRequest request) {
        ...
        boolean flag = userService.authorityUserRole(id,roleId);
        return flag ? new Message().ok("success","") : new Message().error("授权角色失败");
    }

    //@ApiOperation(value = "删除已经授权的用户角色",httpMethod = "DELETE")
    @DeleteMapping("/authority/role/{id}/{roleId}")
    public Message deleteAuthorityUserRole(@PathVariable String id, @PathVariable Integer roleId) {
        return userService.deleteAuthorityUserRole(id,roleId) ? new Message().ok("success","") :  new Message().error("删除已经授权的用户角色失败");
    }
